...
Both fitting engines return the same `calibration_result` object, so
all downstream code — plots, extractors, detection-limit functions —
works identically regardless of which engine produced the result.
...
Spans **two antigens** (`alpha` and `beta`) × **three replicate plates**,
giving six `curve_id` values (1–6). The `alpha` curves were simulated
with a Gompertz model; the `beta` curves with a 5PL model.
...
`preprocess_standards()`; the model functions themselves are
scale-agnostic.
...
Asymmetric without requiring a fifth parameter — skewed toward the
upper asymptote. Often the best fit for Luminex data.
...
`is_log_independent = TRUE`, `loglogistic4` is algebraically
equivalent to `logistic4` and is silently dropped by the fitting
engines to avoid redundancy.
...
must be applied upstream via `curveRcore::preprocess_standards()`.
...
1. **`compute_concentration()`** — converts the `dilution` column to
   absolute concentration using the undiluted standard concentration;
   optionally log₁₀-transforms the result.
2. **`correct_prozone()`** — dampens the hook effect at high
   concentrations by compressing post-peak signal toward the peak.
3. **`perform_blank_operation()`** — applies one of five blank
   strategies: `"ignored"` (default), `"included"` (append geometric
   mean as an extra point), `"subtracted"`, `"subtracted_3x"`, or
   `"subtracted_10x"`.
4. **`compute_log_response()`** — applies `log10()` to the response.
   Values ≤ 0 are floored to 1 % of the minimum positive value *among the
   included points* before transformation.

> **Key contract:** After `preprocess_standards()`, the column named
> by `independent_variable` holds log₁₀ concentration and the response
> column holds log₁₀ response. These are the scales the fitting
> engines expect.
...
`preprocess_standards()` understands an optional `include_col` (default
`"included"`): a logical column marking which rows entered the fit
(`TRUE`) versus which were masked out (`FALSE`). The rule is simple and
strict:

> **Statistics from the included points; transforms applied to every row.**
> The prozone peak, the blank geometric mean, the adaptive log floor, and
> the minimum-concentration anchor are all computed from the *included*
> rows only. The resulting transforms are then applied to *all* rows, so
> masked points land on exactly the same axes as the fitted points without
> ever influencing them.

Masked rows are never dropped — they are transformed and returned so that
downstream code (e.g. an app overlaying the observed points on the fitted
curve) can plot them on the same axes and colour them by status. If the
column is absent, every row is treated as included and the output is
identical to the column-free behaviour. Fitting engines are expected to
receive only the included subset:

```{r included-subset, eval = FALSE}
...
`exclusion_reason`), so a consumer can reconstruct the observed curve with
perfect fidelity without re-deriving any transform.

Note that **blanks are never subtracted automatically**. Under the default
`blank_option = "ignored"` the standard responses are left as-is and the
returned blanks are transformed for display only. Only the explicit
`"subtracted"`, `"subtracted_3x"`, and `"subtracted_10x"` options subtract the
included-blank geometric mean from the standards; `"included"` adds it as an
extra low-concentration point.

## Adaptive constraint profile

`adaptive_constraint_profile()` is called internally before fitting to
classify the assay signal into one of three scale classes and derive
appropriate parameter bounds for the NLS or prior specification:
...
The scale class drives bound widths for `b` (slope) and `g`
(asymmetry), and how far beyond the data range the inflection point
`c` is allowed to roam. Calling this function directly is not usually
necessary, but it is exported for custom constraint workflows.
...
of class `calibration_result`. The structure is defined here in
curveRcore; the fitting engines populate it.

```{r cr-overview, eval = FALSE}
...
that failed to converge have an entry, which is important for
...
gate failure — a useful diagnostic for problem plates.

```{r selection-access, eval = FALSE}
...
points, configurable via `n_grid`). It is the primary input for
plotting the calibration curve and for computing quantification limits.
...
`se_concentration` is the uncapped, modelling-scale quantity and is
what downstream variance/weighting models (e.g. `curveRweights`)
should consume. `pcov` is censored at `cv_x_max` and is intended for
human-readable QC only.

Use `pcov_from_se()` and `se_from_pcov()` — the single canonical
implementation of this conversion — rather than reimplementing it:
...
`compute_detection_limits()`, either automatically inside the fitting
pipeline or post-hoc:

```{r compute-dl, eval = FALSE}
...
**`$mdc_rdl`** — minimum detectable concentration and reliable
detection limit, derived by inverting CI-modified versions of the
fitted curve:
...
AIC and LOO-CV measure forward-fit quality — how well the model
predicts the response values in the training data. They are completely
blind to whether the fitted model can **back-calculate concentration**
reliably. A model whose asymptote estimate is sitting on a constraint
boundary, or whose covariance matrix is nearly singular, can have a
lower AIC than a well-identified competitor purely by overfitting.
...
All gates are evaluated by `assess_model_eligibility()`:
...
means the optimiser could not find an interior minimum — AIC
asymptotics do not hold at the boundary.
...
After assessment, `select_best_eligible()` implements the following
decision tree:
...
The fallback ensures a result is always returned. When `fallback =
...
Every precision grid includes a `d2y_dx2` column: the second
derivative $d^2(\log_{10} y) / d(\log_{10} x)^2$ computed from the
grid points with zero additional model evaluations via
`enrich_grid_with_d2y()`.

For a monotone increasing sigmoid this derivative has a peak below
...
- The **pcov-LOQ** (`elig$lloq`, `elig$uloq`) is precision-based:
  it identifies the concentration range where back-calculation %CV
  stays below `pcov_threshold`. It incorporates parameter uncertainty
...
the object internals directly — the extractors dispatch automatically
to both the single-plate and multiplate classes.

## `tidy_samples()` — sample predictions

```{r tidy-samples, eval = FALSE}
# Single plate
s <- tidy_samples(cr)
...
`calibration_result_multiplate` object — a named list with `$meta`
and `$plates`, where each element is a complete single-plate
`calibration_result`.

```{r multiplate, eval = FALSE}
...
3. **Preprocessing** — `preprocess_standards()` applies concentration
   computation, prozone correction, blank handling, and log₁₀
   transforms in a fixed, reproducible order.
...
5. **Eligibility gating** — `assess_model_eligibility()` and
   `select_best_eligible()` intercept unidentified or
   boundary-constrained models before AIC / LOO ranking.

6. **Result class and extractors** — `calibration_result` (single
   plate) and `calibration_result_multiplate`; tidy extractors
   `tidy_samples()` and `tidy_grid()`; pcov / SE conversion
   utilities; detection-limit functions; and cross-method comparison
   helpers.
