# curveRcore ![curveRcore logo](reference/figures/logo_small.png)

**curveRcore** is the shared engine of the curveR suite. It provides the
forward models, preprocessing pipelines, analytical inverses and
gradients, eligibility gating, detection limits, and the
`calibration_result` S3 class that both `curveRfreq` and `curveRbayes`
return. All fitting engines consume and produce the same objects, so
switching between frequentist and Bayesian approaches — or comparing
them directly — requires no data wrangling.
...
making any downstream code — plots, extractors, weight models —
...
applies four steps in order before any model is fitted. Both
`curveRfreq` and `curveRbayes` call this function identically,
...
result <- preprocess_standards(
  ...
  apply_prozone        = TRUE,
  include_col          = "included"      # optional logical mask column
)

preprocessed <- result$data          # all standard rows, transformed
blanks_out   <- result$blanks        # all blank rows, transformed (or NULL)
stats        <- result$derived_stats # set-level statistics (see below)
# $concentration now holds log10-concentration
# $mfi now holds log10-MFI; $assay_response_raw keeps the pristine response
```

| Step | Function | What it does |
|----|----|----|
| 1 | [`compute_concentration()`](https://immunoplex.github.io/curveRcore/reference/compute_concentration.md) | Converts `dilution` column to absolute concentration; optionally log₁₀-transforms |
| 2 | [`correct_prozone()`](https://immunoplex.github.io/curveRcore/reference/correct_prozone.md) | Dampens the hook (prozone) effect at high concentrations |
| 3 | [`perform_blank_operation()`](https://immunoplex.github.io/curveRcore/reference/perform_blank_operation.md) | Applies the chosen blank strategy |
| 4 | [`compute_log_response()`](https://immunoplex.github.io/curveRcore/reference/compute_log_response.md) | log₁₀-transforms the response; floors non-positive values adaptively |

**Mask-aware.** An optional `include_col` (default `"included"`) marks
which rows entered the fit. Every set-level statistic — the prozone
...
only, while the transforms are applied to *all* rows. Masked rows are
transformed and retained (never dropped) so they share the fitted
points’ axes; feed a fitter `result$data[result$data$included, ]` and
the fit is byte-identical to one that never saw the masked rows. When
the column is absent, every row is treated as included.

[`preprocess_standards()`](https://immunoplex.github.io/curveRcore/reference/preprocess_standards.md)
returns `data`, `blanks` (transformed onto the standards’ response
floor, or `NULL`), `antigen_fit_options`, and `derived_stats`
...
`blank_option = "ignored"` the standard responses are untouched and the
returned `blanks` are transformed for display/persistence only.
Subtraction of the included-blank geometric mean (1×/3×/10×) happens
*only* when the caller explicitly selects `"subtracted"`,
`"subtracted_3x"`, or `"subtracted_10x"`; `"included"` instead adds the
blank mean as an extra fit point.
...
structure is identical regardless of which engine produced it:
    ...
    ├── $standards    preprocessed standard points (all rows, with `included` and
    │                 `assay_response_raw`) as fed to / masked from the fit
    ├── $blanks       preprocessed blank points (all rows, with `included`), or NULL
...
Before AIC or LOO-CV ranking, every fitted model passes through four
gates. A model that fails any gate cannot be selected as the best model,
preventing an unidentified or boundary-constrained fit from winning
...
can be called automatically inside the fitting pipeline or post-hoc on
any existing result:
...
- Full function reference:
  `https://immunoplex.github.io/curveRcore/reference/`
- Getting started vignette:
  [`vignette("getting-started", package = "curveRcore")`](https://immunoplex.github.io/curveRcore/articles/getting-started.md)
- Model family overview:
  `vignette("model-overview", package = "curveRcore")`
