...
**Mask-aware.** An optional `include_col` (default `"included"`) marks which
rows entered the fit. Every set-level statistic — the prozone peak, the blank
geometric mean, the adaptive log floor, and the minimum-concentration anchor —
is computed from the *included* rows only, while the transforms are applied to
*all* rows. Masked rows are transformed and retained (never dropped) so they
share the fitted points' axes; feed a fitter `result$data[result$data$included, ]`
and the fit is byte-identical to one that never saw the masked rows. When the
column is absent, every row is treated as included.

`preprocess_standards()` returns `data`, `blanks` (transformed onto the
standards' response floor, or `NULL`), `antigen_fit_options`, and
...
returned `blanks` are transformed for display/persistence only. Subtraction of
the included-blank geometric mean (1×/3×/10×) happens *only* when the caller
explicitly selects `"subtracted"`, `"subtracted_3x"`, or `"subtracted_10x"`;
`"included"` instead adds the blank mean as an extra fit point.
