...
-   **`pcov` is a capped percent %CV; `se_concentration` is the uncapped
    ...
    `curveRweights`) must use **`se_concentration`**, not `pcov` — the
    cap destroys the precision gradient. The single
    `pcov ↔ se_concentration` conversion lives in **one** core helper
    (`pcov_from_se()` / `se_from_pcov()`); never reimplement it.
-   **Design-group metadata is not in the core object.** Columns like
    `timeperiod` / `cohort_arm` appear in `$samples` **only** if they
    were columns in the `samples` data frame passed to the fitting call
    (the engine carries the original sample columns through). Code that
    needs design groups must require them on the input or join them by
...
2.  **Cross-package accessors live in `curveRcore`.** The canonical
    extractors `tidy_samples()` and `tidy_grid()` and the
    `pcov_from_se()` / `se_from_pcov()` helpers are owned by core.
    Consumers call them; they do not reach into raw object internals or
    re-derive these themselves.
...
6.  **Stan is a `Suggests`, never an `Imports`.** `curveRbayes` and
    ...
    (`cmdstanr::install_cmdstan()` + the install-docs URL), never a
    low-level stack trace. Any feature gated on Stan must be reflected
    in the capability matrix in the hub `installation` article.
...
-   **Adding a column to `$grid` / `$samples`** is additive and
    non-breaking *if* both engines emit it and no consumer assumes a
    fixed column set. Land it in `curveRcore` first (constructors +
    extractors), then in both engines, then announce it in the hub
    `NEWS.md`.
...
-   **New core helper that a spoke needs** must be exported from
    `curveRcore` and released (even a minor bump) *before* the spoke
    that depends on it can declare the new `curveRcore (>= x.y.z)`
    minimum in its `DESCRIPTION`.
...
-   **Scheme:** coordinated `major.minor.patch` across all four
    packages.

-   **Major:** bump **all four together** on a breaking contract change
    (a `$grid`/`$samples` shape change, a removed/renamed field, a
    changed default that alters results).

-   **Minor / patch:** may diverge per package. Additive core work (new
    extractor, new helper, a new additive column) is a **minor** core
    bump and does not force a major elsewhere.

-   **Compatibility matrix:** every release records the exact four
    package versions and notable changes in the hub `NEWS.md`. Template:
...
-   **Release flow:** tag a spoke release → the spoke's GitHub Actions
    release workflow fires a `repository_dispatch` to
    `immunoplex/curveR` (`<spoke>-release`) → the hub rebuilds and
    redeploys its aggregated pkgdown site. Don't hand-rebuild the hub
    site.

-   **CRAN / r-universe:** `curveRcore`, `curveRfreq`, and the `curveR`
    meta-package target CRAN; the two Stan packages (`curveRbayes`,
    `curveRweights`) live on r-universe + GitHub because `cmdstanr` is
    not on CRAN. The meta-package's `Suggests` (not `Imports`) on the
    Stan packages is what keeps a CRAN-only install valid — do not "fix"
    this by promoting them to `Imports`.
...
-   Roxygen for every exported object, with cross-package `@seealso`
    links so the pkgdown reference forms a web (e.g. a fitting function
    points at `curveRcore::preprocess_standards()` and at the other
    engine).
...
-   **The umbrella / methods-comparison vignette lives only in the
    hub.** It is the one place the whole ecosystem runs together;
    keeping it off the spokes is deliberate and drives users to the hub.
    Do not duplicate it into a spoke.
-   Stan-backed vignettes use the agreed precompute/caching strategy so
    CI site builds stay tractable (see the hub's site README / Issue-2
    plan).
...
-   [ ] No dependency edge points the wrong way (§3.1); core gained
    nothing spoke-specific.
-   [ ] Shared extractors/helpers were used, not reimplemented (§3.2);
    `pcov`/`se` handling routes through the core helper (§2).
-   [ ] If `$grid` / `$samples` shape changed: both engines updated,
    additive vs breaking classified, hub `NEWS.md` updated, version
    bumps planned (§4–§5).
-   [ ] Naming/style conventions followed (§3.5); freq and Bayes output
    columns still match.
...
-   [ ] `devtools::document()`, `devtools::test()`, `devtools::check()`,
    `pkgdown::check_pkgdown()` all clean.
-   [ ] New/changed exports documented with `@seealso` cross-links;
    vignette placement respects §8.
...
-   Anything touching the **shared contract, the dependency graph,
    versioning, or the hub** goes on the `immunoplex/curveR` hub repo,
    so the discussion is visible to all four packages.
