...
plate separately (`minpack.lm::nlsLM`, base `nls` port fallback); there
is **no pooling, no hierarchy, no population parameters.** Uncertainty
is propagated by the **delta method** from `coef(fit)` + `vcov(fit)`
(`predict_samples_freq`); there is no native draws/bootstrap. Residual
SE is `summary(fit)$sigma`.
